...
struct nodoEs *inicio=NULL;
struct nodoEs *fin=NULL;

void registrarEsp(){
	char salto;
	struct nodoEs *nuevo;
	nuevo=(struct nodoEs*)malloc(sizeof(struct nodoEs));
...
void nuevoEsp (char nombre[], char apellidop[], char apellidom[], char especialidad[], char pw[]){
	struct nodoEs *aux;
	struct nodoEs *insertaEsp;
	insertaEsp=(struct nodoEs*)malloc(sizeof(struct nodoEs));
	aux=inicio;
...
int main(){
	...
	gotoxy(18,11);printf("Seleccion: ");
    	...
    	gotoxy(27,11);printf("     ");
    	gotoxy(18,11);printf("Seleccion: ");
    	gotoxy(28,11);scanf("%i",&solicitud);
	}
		switch (solicitud){
				case 1:
					system("clear");
